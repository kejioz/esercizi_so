#include "semafori.h"
#include "monitor_signal_and_continue.h"
...
//Starvation entrambi MONITOR sec
void lettoreMonitorEntrambi(LettScrittMonitorEntrambi* ls){

	//Come prima cosa entro nel monitor
	enter_monitor (&ls->m);
	
		//Sono un lettore, posso stare qui? Ovvero, mica ci sono scrittori? Check su cv RCORDANDO che è signal and continue
		...
		//QUI A DIFFERENZA DI HOARE NN SEGNALO UN ALTRO LETTORE
...
void scrittoreMonitorEntrambi(LettScrittMonitorEntrambi* ls){
	...
	enter_monitor(&ls->m);
		...
		//CI sono scrittori in attesa sulla queue? Se sì segnalo loro, altrimenti segnalo TUTTI I ELTTORI
