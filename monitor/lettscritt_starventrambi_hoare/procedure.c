...
int Leggi(struct LettScritt * ls) {
    ...
    printf("Lettura - ingresso monitor\n");
        ...
        if ( ls->numero_scrittori > 0){
            ...
            //Questo è un signal and wait. Quindi quando lo scrittore segnala, va in wait
            //Quindi vuol dire che lo scrittore può risvegliare solo un lettore
        ...
        //Siccome questa è una signal and wait, lo scrittore risveglia solo un lettore
        //Quindi ogni lettore deve fare una signal sugli altri lettori
...
void Scrivi(struct LettScritt * ls, int valore) {
    ...
    enter_monitor( &(ls->m) );
        ...
        if (queue_condition( &(ls->m),VARCOND_SCRITTORI)){
            ...
            //Nota che faccio una sola signal, saranno poi i lettori a svgliare gli altri
